...
void convertPose2T(int i_ref, const poseimagearraymsg::poseImageArrayConstPtr msg, Mat &P_ref){
	...
	r_x = msg->poseArray.at(i_ref).pose.position.x;							
	r_y = msg->poseArray.at(i_ref).pose.position.y;							
	r_z = msg->poseArray.at(i_ref).pose.position.z;							
	r_qx = msg->poseArray.at(i_ref).pose.orientation.x;						
	r_qy = msg->poseArray.at(i_ref).pose.orientation.y;						
	r_qz = msg->poseArray.at(i_ref).pose.orientation.z;						
	r_qw = msg->poseArray.at(i_ref).pose.orientation.w;						
